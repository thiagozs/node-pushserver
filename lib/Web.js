...
// Middleware
app.use(compression()); //new change
app.use(bodyParser.json()); //new change
...
if (config.get('useAuth')) {
    var basic = auth.basic({
        realm: "Node-Pushserver Area."
    }, function(username, password, callback) { // Custom authentication method. change if your needs
        callback(username === config.get("basicAuth").user && password === config.get("basicAuth").pass);
    });
        ...
        var fetchUsers = users ? pushAssociations.getForUsers : pushAssociations.getAll,
            callback = function(err, pushAssociations) {
                ...
                if (target !== 'all') {
                    ...
                    pushAssociations = _.where(pushAssociations, {
                        'type': target
                    });
                }
